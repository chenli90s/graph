...
// import demoRelation from './data/demo_relation.json';
...
class D3Graph {
  ...
  highlightLine = (index) => {
    ...
    const nodesIds = nodes.map(i => i.id);
    ...
    this.linksBox
      ...
      const nodesData = this.nodesBox.selectAll('.node')

      nodesData.select('circle').attr('fill', (d) => {
        if (nodesIds.includes(d.id)) {
          return colorMap[d.idtype];
        }else{
          return '#a0a0a0';
        }
  ...
  dragended = (d) => {
    d.fx = d3.event.x;
    d.fy = d3.event.y;
    if (!d3.event.active) this.simulation.alphaTarget(0).stop();
  }
}
